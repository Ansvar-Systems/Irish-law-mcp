/**
 * search_eu_implementations — Search EU directives/regulations with Irish implementation info.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_irish_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS irish_statute_count,
  ...
  if (input.has_irish_implementation !== undefined) {
    sql += input.has_irish_implementation ? ` HAVING irish_statute_count > 0` : ` HAVING irish_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    irish_statute_count: number; primary_implementations: string | null;
  }
